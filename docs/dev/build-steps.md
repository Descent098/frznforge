...
| Metadata reuse | `ingest.skipMetaRefetches`, default `false` | `remote.go:1030`, `remote.go:1085` | The only skip here that is **not** self-healing, and the only one that can change the artifact — which is why it is off by default and why it fires only after `providerMetaUsable` confirms the cached record is a completed answer (name, web URL, clone URL). What it serves is exactly what the call would have returned when it was made; what it cannot promise is that the answer is still current. Time-bounded skips heal; this one is served until `--refresh-meta`, `--no-cache` or `"fetch": "always"`. Git and releases are untouched. |
