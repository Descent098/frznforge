...
* **`ingest.skipMetaRefetches` stops re-asking a provider for descriptions it already has.** With it on, a repository whose cached provider record is complete — a name, a web URL and a clone URL, the signature of a call that actually succeeded — serves that record instead of spending a request on it. On a 73-repository account that is 73 requests a build, which is the difference between fitting inside a quota and running out partway down the list. **Releases and git are still fetched**, so new commits and new releases still appear; only the description-and-topics record is frozen. Off by default, and `--refresh-meta`, `--no-cache` or `"fetch": "always"` all force a refresh.

  The cost is stated plainly in the documentation because it is real: every other skip in the ingest is time-bounded and heals itself, and this one serves the cached record until you refresh it. It raises no warning — a warning is artifact bytes, and one here would make the published file depend on how warm the building machine's cache happened to be — so the console line and the run log are where staleness surfaces instead, and `ingest.failOnDegraded` stops saying anything about metadata age while it is on.
...
* **A wide image in rendered markdown ran off the page.** Nothing bounded `img` anywhere in the stylesheet, so a README containing a screenshot at its original size — 3,861 pixels wide, in the case that surfaced it — overflowed its card and dragged the whole page's horizontal scrollbar out with it. Bounded to the column now, with `height: auto` so the aspect ratio survives being narrowed. The rule sits on `.hf-md`, the rendered-markdown container, rather than on the README card alone: the same markdown renders in notes, on organization and profile pages, in releases and in any markdown file opened from a tree, and every one of them had the same overflow — the page that surfaced it was a tree page. Rendered mermaid diagrams already had this treatment two lines above; images were simply missed.
